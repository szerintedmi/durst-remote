...
namespace DurstProto
{
  ...
  // Handler type for TM state broadcast messages
  using TmStateBroadcastHandler = void (*)(const MsgTmStateV1 &msg);
  ...
  // Register TM-state broadcast handler and ensure ESP-NOW recv callback is attached
  ...
  // Send current TM state to broadcast MAC
